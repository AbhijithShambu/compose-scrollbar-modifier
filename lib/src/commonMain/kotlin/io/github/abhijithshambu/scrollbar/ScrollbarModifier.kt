package io.github.abhijithshambu.scrollbar
...
import io.github.abhijithshambu.scrollbar.foundation.ScrollbarLayout
import io.github.abhijithshambu.scrollbar.foundation.ScrollbarLayoutScope
import io.github.abhijithshambu.scrollbar.foundation.ScrollbarMeasureAndDraw
import io.github.abhijithshambu.scrollbar.foundation.ScrollbarMeasurementResult
...
import io.github.abhijithshambu.scrollbar.foundation.applyPadding
import io.github.abhijithshambu.scrollbar.foundation.drawRoundRect
import io.github.abhijithshambu.scrollbar.foundation.isTransparent
import io.github.abhijithshambu.scrollbar.foundation.toStroke
...
private class ScrollbarModifierNode(
    var scrollState: ScrollableState,
    ...
    override fun onAttach() {
        ...
        scrollbarState.scrollTo = getScrollTo()
    ...
    fun getValue(): Int =
        when (val scrollState = scrollState) {
            ...
            is LazyListState -> {
                val approxItemSize = getApproxItemSize(scrollState)
                val maxApproxFirstVisiblePossible =
                    scrollState.layoutInfo.totalItemsCount - scrollState.layoutInfo.visibleItemsInfo.size + 1
                val factor = scrollState.firstVisibleItemIndex.toFloat() / maxApproxFirstVisiblePossible
                scrollState.firstVisibleItemScrollOffset +
                    (scrollState.firstVisibleItemIndex) * approxItemSize +
                    (scrollbarState.viewPortLength * factor).roundToInt()
            }

            else -> 0
        }

    private var estimatedMaxvalue: Int? = null

    fun getMaxValue(): Int =
        when (val scrollState = scrollState) {
            is ScrollState -> scrollState.maxValue
            is LazyListState -> {
                if (estimatedMaxvalue == null) {
                    estimatedMaxvalue = scrollState.layoutInfo.totalItemsCount *
                        getApproxItemSize(scrollState)
                }
                estimatedMaxvalue!!
            }
            else -> 0
    ...
    private fun getApproxItemSize(state: LazyListState): Int {
        val visibleItems = state.layoutInfo.visibleItemsInfo
        return (visibleItems.sumOf { it.size }) / (visibleItems.size)
    }

    fun getScrollTo(): suspend (offset: Int) -> Float =
        when (val scrollState = scrollState) {
            is ScrollState -> scrollState::scrollTo
            is LazyListState -> (
                { offset: Int ->
                    val currentOffset = getValue()
                    val diffOffset = if (currentOffset < offset) {
                        currentOffset + offset
                    } else {
                        -(currentOffset + offset)
                    }
                    scrollState.scrollBy(diffOffset.toFloat())
                }
            )
            else -> ({ _: Int -> 0f })
        }
